...
###---------
    ...
    #     if not media_player:  # TODO self.cehck_alexa_service?
    #         media_player = [m for m in self.cehck_alexa_service if self.friendly_name(m).lower() in user_player]
    ...
    #                 self.lg(f'INFO DA GRUPPO: {mpu} -> {self.get_state(mpu, attribute="entity_id")}')
    #                 self.lg(f'INFO DA SENSORE: {mpu} -> {self.get_state(mpu)}')
    #                 self.lg(f'INFO DA MEDIA PLAYER: {mpu} -> {mpu}')
    #                 self.log(f"Invalid group, sensor or player ENTITY-ID ({mpu})", level="WARNING")
    #             self.lg(f'INFO DA FRIENDLY: {mpu}')
    #         self.log(f"No media player found. I use the default one. ({media_player})", level="WARNING")
    #     self.lg(f"GET PLAYER: {media_player}")
    #     self.lg(f"SET REMOVE PLAYER: {list(set(media_player))}")
    ...
    #     media_player = []
    #     user_player = self.converti(str(user_player.lower()))
    #     for mpu in user_player: #                def if_entity(self): 
    #         if self.entity_exists(mpu):
    #             self.log(f"CALL DIRETTO ENTITY EXIST {mpu}", level="WARNING")
    #             self.mp(media_player, mpu)
    ...
    #             #     self.log(f"Invalid group, sensor or player ENTITY-ID ({mpu})", level="WARNING")
    #         else:
    #             # alexa_option = self.get_state(self.alexa_select_media_player, attribute="options")
    #             # alexa_option = [self.friendly_name(mpu) for x in alexa_option]
    #             # self.log(f"alexa_option {alexa_option}", level="WARNING")
    #             # media_player.extend([m for m in self.cehck_alexa_service if self.friendly_name(m).lower() in mpu]) # = friendly self.get_state(self.alexa_select_media_player, attribute="options")
    #             for m in self.list_select:
    #                 if self.friendly_name(m).lower() in mpu:
    #                     self.log(f"CALL NAME TROVATO call entity {m}", level="WARNING")
    #                     self.mp(media_player, m)
    #                     # if "group." in m:
    #                     #     media_player.extend(self.get_state(m, attribute="entity_id"))
    #                     # elif "sensor." in m:
    #                     #     media_player.append(self.get_state(m))
    #                     # else:
    #                     #     media_player.append(m)
    #                     break

    #     self.lg(f"PRIMA DI SET PLAYER: {media_player}")
    #     if not media_player:
    #         media_player.append(self.get_state(self.alexa_sensor_media_player))
    #         self.log(f"No media player {user_player} found. I use the default one. ({media_player})", level="WARNING")
    #     media_player = list(set(media_player))
    ...
    # def get_entity_id(self, friendlyname):
    #     states = self.get_state()
    #     for entity in states:
    #         if entity["friendly_name"] == friendlyname:
    #             return entity
    #     return None
...
############ NOTIFIER 
        # notifier = str(alexa.get("notifier"))
        # if notifier and notifier in self.cehck_alexa_service:
        #     alexa_player = self.player_get(notifier.replace("alexa_media_", "media_player."))
        # else:
        #     notifier = self.alexa_service
        #     alexa_player = self.player_get(alexa.get("media_player", self.get_state(self.alexa_sensor_media_player)))
